...
Purpose
  This project looked at 2017 and 2018 stock data in order to analyze returns using VBA code to loop through the data for Steve so data driven decisions could be made for his parentds regarding appropriate stock picks. A refactored code was then introduced to see whether the time to execute and run the code was more efficient. Then, an analysis was done to discuss the pros and cons for code refactoring, as well as the challenges that one may face with refactoring.
